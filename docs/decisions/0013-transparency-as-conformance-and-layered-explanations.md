...
**Schema-level transparency hooks.** `match-response.schema.json` requires `decision_explanation` at both layers and `exclusion_summary` when exclusions exist. `handoff-component2.schema.json`, `referral-proposal.schema.json`, and `referral-connection.schema.json` add optional `decision_explanation` for lifecycle events. `audit-event.schema.json` adds optional reverse reference `decision_explanation_id` so the audit log can cross-reference the explanation it corresponds to.
...
- Every relevant response shape carries `decision_explanation` (required on `match-response.schema.json` at both layers; optional on `handoff-component2`, `referral-proposal`, `referral-connection` for lifecycle events). Phase 6 final validation includes a check that every response shape that should have it does have it.
...
- The professional-facing endpoint is one of v0.8.2's three named v0.8.3-deferred items (the others are Component 3 dense-cluster pattern detection and engagement liveness response-rate tracking). The v0.8.2 obligation remains in force; only the endpoint implementation may defer.
...
- Schema updates: [`match-response.schema.json`](../../spec/v0.8/schemas/match-response.schema.json) (Layer 1 + Layer 2 + Layer 3 — `decision_explanation` required at both layers; `exclusion_summary` added); [`handoff-component2.schema.json`](../../spec/v0.8/schemas/handoff-component2.schema.json), [`referral-proposal.schema.json`](../../spec/v0.8/schemas/referral-proposal.schema.json), [`referral-connection.schema.json`](../../spec/v0.8/schemas/referral-connection.schema.json) — optional `decision_explanation` for lifecycle events; [`audit-event.schema.json`](../../spec/v0.8/schemas/audit-event.schema.json) — optional reverse reference `decision_explanation_id`.
