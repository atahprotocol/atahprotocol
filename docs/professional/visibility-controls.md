...
## A note on referral relationships

If you have established Type 2 referral relationships with other professionals through ATAH, withdrawing your record from matching does not automatically end those relationships. Type 2 relationships persist on the professional record as a personal, informal asset — they are part of how you are represented to AI systems, not a separate database the registry can recompose. Withdrawal stops new consumer matches being routed to you; existing referral relationships continue until you (or the other professional) ends them through the standard relationship-management flow.

For full detail on Type 2 referral relationships, see the README section on referral relationships between professionals.
