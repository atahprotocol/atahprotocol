...
Professionals using ATAH face a question their existing marketing tools don't answer: *how does my profile actually rank when an AI system queries on behalf of someone in my category?* The matching engine is publicly documented (rubric, components, weights) and the `_provenance` map and match response carry per-component contribution data — so the underlying signals are not secret. But assembling them into a useful self-service answer requires running representative queries and breaking down the result.

The "Your visibility" report is the planned professional-portal feature that does this. It runs representative queries against the professional's own profile, surfaces how the professional ranks, and shows the per-component contribution breakdown — so the professional can understand their AI-discoverability without manual AEO work or third-party tooling.

The report is descriptive, not prescriptive. It tells a professional how they currently rank; it does not tell them what to do about it. Where contributions are low because of factors the professional can affect (profile completeness, scope details, partner verification), those become useful inputs to their own decisions about updating the profile or joining a body. Where contributions are low because of factors outside their control (no partner integration in their category yet, low review volume in a category where review weight is capped), the report makes that visible too — which is itself useful information.
...
**The ranking.** Where the professional appeared in the response — first, fifth, not in the shortlist — and the total candidate pool size for that query. "Fifth of fifteen" is more informative than "fifth" alone.

**The contribution breakdown.** For each appearance, the per-component scores driving the ranking: relevance, verification quality (split into the four sub-components — credential and standing, enhanced verification, review platform signals, scope completeness), availability and response time, profile completeness, and inbound referral signal. Each is shown with the actual contribution to the match score, not just the input value.

The breakdown also shows the `verification_confidence` signal (single-source vs multi-source), the partner-class summary for the professional's verifications, and any active enhanced verification records. These are the signals the AI platform sees on every match response — the report shows the professional what their own match response looks like.

## What the report does not show

A short list of deliberate omissions:

**Specific competing professionals.** The report is about the professional's own positioning, not about other professionals. Where the professional ranked fifth, the report does not name who ranked first through fourth. AI-discoverability transparency is for the professional reading their own report; competitive intelligence on other professionals is not the purpose.

**Aggregate query volume against the professional's profile.** The report shows representative queries, not actual queries the registry has handled. Real query patterns would expose information about consumers and AI platform integrations that doesn't belong in a per-professional report. **In v0.8.2 this is codified as a normative MUST NOT in spec §11A.4 (per F-18):** professional-facing visibility explanations MUST NOT expose actual query-count data, query-history data, observed demand patterns, or any per-query information derived from consumer or platform traffic; they MUST present representative explanation categories at the category/jurisdiction level, derived from the implementation's documented rules and the professional's own profile data. The existing posture this document already describes is preserved and tightened as a spec-level normative rule. The professional-facing visibility view delivered by the new `GET /v1/professionals/me/visibility-explanations` endpoint (whether implemented in v0.8.2 or as a v0.8.3 fast-follow per ROADMAP.md) is rules-derived for the same reason — even aggregated query counts can leak in low-volume categories.

**Match decisions made by AI platforms.** Once a match response leaves ATAH, the AI platform applies its own contextual layer — re-ordering, filtering, presenting only some of the shortlist. The report shows what ATAH returned, not what the AI platform did with the response. Platform-side decisions are platform business.
...
## Contribution breakdown — what each component means in plain language

For professionals who haven't read the matching engine specification (most of you), here is what each contribution component represents:

**Relevance.** How well the professional's declared scope (category, matter types, specialisms, jurisdictions, languages) fits the query. Improvable by making your declared scope more accurate — too narrow misses queries you could handle; too broad weakens relevance scoring on queries you actually fit.

**Verification quality.** The four sub-components together:
- *Credential and standing verification.* Weighted by the partner's `vetting_strength` — regulatory verification weighs more than open-membership-body verification. Mostly outside the professional's control; reflects which authoritative sources cover the category and whether the relevant partner agreement is in place.
- *Enhanced verification.* Increases when an active independent-verifier verification record exists for the professional. Within the professional's control: enhanced verification can be commissioned by the professional, an employer, or a professional body offering it as a member benefit.
- *Review platform signals.* Above the volume threshold, time-decayed, weighted by review platform class. Capped per category — for high-stakes regulated categories, ≤0.10 of the verification quality score regardless of review volume. Reviews complement but do not substitute for credential verification.
- *Scope completeness.* Split into declared scope and verified scope. Only verified scope materially contributes. Improvable by completing partner-verifiable scope fields (specialisms confirmed by the relevant body, lines of authority for insurance, etc.).

**Availability and response time.** Whether the professional is currently accepting introductions, the declared response-time commitment, and timezone alignment with the query. Improvable by keeping availability current and setting realistic response-time commitments.
...
For implementers building a conforming registry, an equivalent feature is encouraged but not protocol-mandated. The matching engine rubric is public, the per-component contribution metadata is in every match response, and the `_provenance` map carries field-level verification status — so a conforming registry has all the inputs needed to build an equivalent report.
...
For the matching engine rubric, see [PRD §8.5](../../PRD-v0_8.md) and [spec §9](../../spec/v0.8/full-spec.md). For the verification quality sub-components and weights, see PRD §8.5. For the `_provenance` map and verification status enum, see [spec §4.2 and §4.3](../../spec/v0.8/full-spec.md). For the partner taxonomy that determines how `vetting_strength` weights credential verification, see [PRD §6.3](../../PRD-v0_8.md). For visibility classes and how they interact with whether the report is meaningful, see [`visibility-controls.md`](visibility-controls.md).
