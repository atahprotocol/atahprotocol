...
- **Non-endorsement by design.** ATAH is not a recommendation engine. It returns provenance-visible candidate sets. The user or calling AI platform remains responsible for selection. Every match response carries a `presentation_disclosure` block making this machine-readable. The non-endorsement stance is structural at the response layer and a conformance obligation at the consumer layer; the protocol relies on the conformance mark for the latter.
